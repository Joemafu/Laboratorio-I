...
int main()
{
    ...
    do
    {
        ...
        fflush(stdin); //limpia la memoria buffer donde se almacena el enter ingresado previamente, que interfiere en la toma del caracter.
        scanf("%c",&cont);
        ...
        if (sexo == 'f' && (flagM == 0 || edad > edadMayor))
        {
            edadMayor = edad;
            altVieja = altura;
        }
        ...
        if (sexo == 'm' && (flagH == 0 || altura < alturaMenor))
        {
            alturaMenor = altura;
            edadMasBajo = edad;
        }
    ...
    alturaHombres = acumuladorAltH / contadorH;
